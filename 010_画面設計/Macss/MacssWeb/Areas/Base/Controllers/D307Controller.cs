...
{
    public class D307Controller : BaseController
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UploadConfirm2(HttpPostedFileWrapper postedFile)
        {
            if (postedFile == null)
            {
                ViewBag.ErrorMessage = "ファイルがありません。";
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UploadResult2(string uploadFileName)
        {
            var list = TempData["UploadedList"] as List<AccountMasterUpload>;
            var resultList = new List<AccountMasterUpload>();
